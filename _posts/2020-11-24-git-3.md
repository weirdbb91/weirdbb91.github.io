 ...
 * `format` 값은 원하는 포맷으로 순서를 정해 출력한다.
```
...
`branch`와 `Merge` 히스토리를 보여주는 아스키 그래프를 출력해주는  
`--graph` 와 함께 쓰면 좋다.  
...
다수의 옵션 사용시 `||`가 아닌  
`&&`를 원한다면 `--all-match` 옵션도 반드시 함께 사용해야 한다.
{: .notice--info}

---

`-S` 옵션은 코드에서 추가/제거된 내용 중 `특정 값의 포함여부`를 검색한다.  
...
디렉토리나 파일 이름으로 그 파일이 변경된 `log`의 결과를 검색할 수 있다.
